...
# Modified from https://github.com/Washington-University/HCPpipelines/blob/master/global/scripts/average_bvecs.py
#
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output", help="Output basename", type=str, required=True
    )
    args = parser.parse_args()

    bvals1, bvals2 = np.split(np.loadtxt(args.bvals), 2)
    bvecs1, bvecs2 = np.split(np.loadtxt(args.bvecs), 2, axis=1)
    ...
    bvals_out = "{0}.bvals".format(args.output)
    bvecs_out = "{0}.bvecs".format(args.output)
    indices_out = "{0}.idxs".format(args.output)
